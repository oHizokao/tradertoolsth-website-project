...
TT.layout = (function () {
  ...
  // ---------- Logo — compact cyan market bars from the approved reference ----------
  function logo(size = 36) {
    return `<svg class="brand__logo" width="${size}" height="${size}" viewBox="0 0 40 40" fill="none" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">
      <rect x="5" y="21" width="5" height="12" rx="2" fill="#1CA7E8"/>
      <rect x="13" y="14" width="5" height="17" rx="2" fill="#19D1C1"/>
      <rect x="21" y="7" width="5" height="22" rx="2" fill="#47E0D2"/>
      <path d="M6 25 15 18l8 3 11-10" stroke="#1CA7E8" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"/>
      <circle cx="34" cy="11" r="2.5" fill="#19D1C1"/>
    </svg>`;
  }
  ...
  function navbar(activeKey = "") {
    ...
    const linksWithDrop = `
      ...
      ${toolsDropdown}
      <a href="signal.html" class="${activeKey === "signal" ? "is-active" : ""}" data-nav="signal">สัญญาณ <span class="nav-mini-chevron">⌄</span></a>
      <a href="news.html" class="${activeKey === "news" ? "is-active" : ""}" data-nav="news">ข่าวสาร</a>
      <a href="knowledge.html" class="${activeKey === "knowledge" ? "is-active" : ""}" data-nav="knowledge">ความรู้ <span class="nav-mini-chevron">⌄</span></a>
